...
root_path = '/Users/wanghuailin/Documents/develop/tensorflow/HWDB'
...
tf.app.flags.DEFINE_integer('max_steps', 200000, 'the max training steps ')
...
tf.app.flags.DEFINE_string('checkpoint_dir', os.path.join(root_path, 'checkpoint'), 'the checkpoint dir')
tf.app.flags.DEFINE_string('train_data_dir', os.path.join(root_path, 'train'), 'the train dataset dir(containing png files)')
tf.app.flags.DEFINE_string('test_data_dir', os.path.join(root_path, 'test_pics'), 'the test dataset dir(containing png files)')
tf.app.flags.DEFINE_string('log_dir', os.path.join(root_path, 'log'), 'the logging path)')
tf.app.flags.DEFINE_boolean('restore', False, 'whether to restore from checkpoint')
tf.app.flags.DEFINE_integer('epoch', 1, 'whether to restore from checkpoint')
tf.app.flags.DEFINE_integer('val_batch_size', 128, 'value on a batch size')
    ...
    for root, sub_folder, file_list in os.walk(data_dir):
        for file_path in file_list:
            if file_path.endswith('.png'):
                filenames.append(os.path.join(root, file_path))
...
def batch_data(file_labels, sess, batch_size=128):
    ...
    #     #print(file_label[1])
    #     try:
    #         int_label = int(file_label[1])
    #     except Exception as err:
    #         print('error:', file_label[0])
    image_list = [file_label[0] for file_label in file_labels]
    label_list = [int(file_label[1]) for file_label in file_labels]
    print('total image size: {0}'.format(len(image_list)))
...
def validation():
    ...
    for i in range(test_steps):
        ...
        labels_batch = []
        labels_max_batch = []
        print('==========start validation on {0}/{1} start=========='.format(i, test_steps))
        ...
        print('==========start validation on {0}/{1} end=========='.format(i, test_steps))
...
def train():
    ...
    test_writer = tf.summary.FileWriter(os.path.join(FLAGS.log_dir, 'val'))
    start_step = 0
    if FLAGS.restore:
        ckpt = tf.train.latest_checkpoint(FLAGS.checkpoint_dir)
        if ckpt:
...
def eval_metric(final_predict_index, groundtruth):
    ...
    for index, val in enumerate(groundtruth):
        ...
        largest_predict = final_predict_index[index][0]
        if val == largest_predict:
            accuracy_cnt += 1
...
def run():
    ...
    if FLAGS.mode == "train":
        ...
        print('Write result into {0}')
        import pickle
        f = open(result_file, 'wb')
        pickle.dump(result, f)
        f.close()
        print('Write file ends')
        ...
        print('the result info label {0} predict index {1} predict_val {2}'.format(1066, final_predict_index,
                                                                                         final_predict_val))
